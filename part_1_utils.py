...
def norm_xcorr(t, a, method=None, trim=True, do_ssd=False):
    ...
    outdims = np.array([a.shape[dd] + t.shape[dd] - 1 for dd in np.arange(a.ndim)])
...
def ndpad(a, npad=None, padval=0):
    ...
    *If npad is not a whole number, padding will be applied so that the
    ...
    padsize = [a.shape[dd] + 2 * npad[dd] for dd in np.arange(a.ndim)]
    b = np.ones(padsize, a.dtype) * padval

    # construct an N-dimensional list of slice objects
    ind = [slice(np.floor(npad[dd]), a.shape[dd] + np.floor(npad[dd])) for dd in np.arange(a.ndim)]
...
# 				'dimensionality of the input array (%i)'
# 				%(len(npad),b.ndim))
# 	ind = [slice(np.floor(npad[dd]),b.shape[dd]-np.ceil(npad[dd])) for dd in np.arange(b.ndim)]
# 	return b[ind]
